...
void loadWallTexture(game_env genv, SDL_Renderer* pRenderer) {
    genv->wall0 = IMG_LoadTexture(pRenderer, "wall0.png");
    genv->wall1 = IMG_LoadTexture(pRenderer, "wall1.png");
    genv->wall2 = IMG_LoadTexture(pRenderer, "wall2.png");
    genv->wall3 = IMG_LoadTexture(pRenderer, "wall3.png");
    genv->wall4 = IMG_LoadTexture(pRenderer, "wall4.png");
    genv->wallu = IMG_LoadTexture(pRenderer, "wallu.png");

    genv->lightbulb = IMG_LoadTexture(pRenderer, "lightbulb.png");
    genv->mark = IMG_LoadTexture(pRenderer, "mark.png");
    ...
    if (event.type == SDL_MOUSEBUTTONDOWN) {
        if (event.button.button == SDL_BUTTON_LEFT) {
            ...
            if (genv->case_x % 2 == 0 && genv->case_y % 2 == 0) {
                ...
                switch (levelID) {
                    case 0:
                        *g = game_load("level1.txt");  // DEBUG change
                        genv->state = "game";
                        genv->actualgame = 1;
                        run = false;
                        break;
                    case 1:
                        *g = game_load("level2.txt");
                        genv->state = "game";
                        genv->actualgame = 2;
                        run = false;
                        break;
                    case 2:
                        *g = game_load("level3.txt");
                        genv->state = "game";
                        genv->actualgame = 3;
                        run = false;
                        break;
                    case 3:
                        *g = game_load("level4.txt");
                        genv->state = "game";
                        genv->actualgame = 4;
                        run = false;
                        break;
                    case 4:
                        *g = game_load("level5.txt");
                        genv->state = "game";
                        genv->actualgame = 5;
                        run = false;
                        break;
                    case 5:
                        *g = game_load("level6.txt");
                        genv->state = "game";
                        genv->actualgame = 6;
                        run = false;
                        break;
                    case 6:
                        *g = game_load("level7.txt");
                        genv->state = "game";
                        genv->actualgame = 7;
                        run = false;
                        break;
                    case 7:
                        *g = game_load("level8.txt");
                        genv->state = "game";
                        genv->actualgame = 8;
                        run = false;
                        break;
                    case 8:
                        *g = game_load("level9.txt");
                        genv->state = "game";
                        genv->actualgame = 9;
                        run = false;
                        break;
